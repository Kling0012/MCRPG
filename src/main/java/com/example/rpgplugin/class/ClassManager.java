...
public class ClassManager {
    ...
    /** プレイヤーのクラス履歴（クラスアップ用） */
    private final Map<UUID, List<String>> playerClassHistory;
    ...
    public ClassManager(PlayerManager playerManager) {
        ...
        this.playerClassHistory = new ConcurrentHashMap<>();
    ...
    public boolean setPlayerClass(Player player, String classId) {
        ...
        if (oldClassId != null && !oldClassId.equals(classId)) {
            playerClassHistory.computeIfAbsent(uuid, k -> new ArrayList<>()).add(oldClassId);
        }
    ...
    public void clearPlayerClass(Player player) {
        ...
        UUID uuid = player.getUniqueId();
        String oldClassId = playerClasses.remove(uuid);
    ...
    public List<String> getPlayerClassHistory(Player player) {
        ...
        List<String> history = playerClassHistory.get(player.getUniqueId());

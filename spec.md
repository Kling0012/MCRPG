...
### フェーズ1: コア基盤（1-2週間）【最優先】
**優先度: 最高**

- [ ] モジュールシステムと依存性管理
  - ModuleManager, IModuleインターフェース
  - DependencyManager（MythicMobs最新版チェック）
  - エラーハンドリングとログシステム

- [ ] データ永続化レイヤー（50-150人対応）
  - DatabaseManager（SQLite接続プール）
  - SchemaManager（自動テーブル作成）
  - PlayerDataRepository（CRUD操作）
  - 3層キャッシュ実装（L1, L2, L3）
...
- [ ] ステータス基盤
  - Stat Enum（STR, INT, SPI, VIT, DEX）
  - StatManager（EnumMap使用）
  - RPGPlayer（プレイヤーラッパー）
  - StatModifier（バフ/デバフ）
...
- [ ] ダメージイベント処理
  - DamageManager
  - EntityDamageEventハンドラー（HIGH優先度）
  - ダメージ倍率適用

- [ ] ダメージ修正システム
  - DamageModifier（属性、クラス補正）
  - クリティカル計算（DEX影響）
  - 防御力計算（VIT影響）
...
- [ ] クラス基盤（重要）
  - RPGClassクラス
  - ClassManager
  - ClassLoader（外部YAML読み込み）
  - StatGrowth（自動+手動配分設定）
...
- [ ] オークション（重要）
  - AuctionManager
  - BiddingSystem（入札10%上乗せ、30-180秒、+5秒延長）
  - 手数料システム
  - 出品/購入/入札コマンド
